...
export default function Signup() {
  ...
  const handleGoogleSignup = () => {
    ...
    window.location.href = "https://zenz-backend.onrender.com/api/auth/google";
  };
      ...
      <div className="bg-white border border-gray-100 rounded-t-3xl px-6 py-8 shadow-2xl max-h-[70vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 p-3 rounded-lg">
            ...
            <div className="space-y-1">
              <div className="flex items-center text-xs">
                <Check size={12} className={`mr-2 ${formData.password.length >= 8 ? 'text-green-500' : 'text-gray-400'}`} />
                <span className={formData.password.length >= 8 ? 'text-green-600' : 'text-gray-500'}>
                  At least 8 characters
                </span>
              </div>
              <div className="flex items-center text-xs">
                <Check size={12} className={`mr-2 ${/[A-Z]/.test(formData.password) ? 'text-green-500' : 'text-gray-400'}`} />
                <span className={/[A-Z]/.test(formData.password) ? 'text-green-600' : 'text-gray-500'}>
                  One uppercase letter
                </span>
              </div>
              <div className="flex items-center text-xs">
                <Check size={12} className={`mr-2 ${/[0-9]/.test(formData.password) ? 'text-green-500' : 'text-gray-400'}`} />
          ...
          <div className="flex items-start space-x-3">
            ...
            <p className="text-sm text-gray-600">
              I agree to the{" "}
              <button className="text-green-900 hover:text-green-700 font-medium">
                Terms & Conditions
              </button>
              {" "}and{" "}
              <button className="text-green-900 hover:text-green-700 font-medium">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-green-900 text-white py-3 rounded-xl font-semibold text-lg flex items-center justify-center transition-all hover:bg-green-800 disabled:opacity-50 mt-6"
          >
            {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button
            ...
            className="flex items-center justify-center py-3 px-4 border border-gray-300 rounded-xl hover:bg-gray-50 transition-all"
          >
          ...
          <button
            disabled
            className="flex items-center justify-center py-3 px-4 border border-gray-300 rounded-xl hover:bg-gray-50 transition-all opacity-50 cursor-not-allowed"
          >
